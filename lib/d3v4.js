...
function zoom() {
  ...
  const imagewidth=window.innerHeight*0.55
  const firstwidth=window.innerWidth/365;
  const maxextent=imagewidth/firstwidth;
  const trans=(window.innerWidth/2)-maxextent/2;
  function dblclicked() {
    if (!filter.apply(this, arguments)) return;
    var t0 = this.__zoom,
        p0 = mouse(this),
        ...
        t1 = constrain(translate(scale(t0, k1), p2, p1), extent.apply(this, arguments), translateExtent);
    noevent$2();
